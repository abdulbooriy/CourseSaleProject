...
async function getAll(req, res) {
    try {
        let courses = await Course.findAll({ include: [{ model: CourseCategory }, { model: Users }] })
        if (!courses) {
...
async function getOne(req, res) {
    try {
        let { id } = req.params
        let course = await Course.findByPk(id, { include: [{ model: CourseCategory }, { model: Users }] })
        if (!course) {
            res.status(400).send({ msg: "Not found!!!" })
        }
...
async function getBySearch(req, res) {
    try {
        ...
        let courses = await Course.findAll({ where: newQuery, include: [{ model: CourseCategory }, { model: Users }] })
        if (!courses) {
            res.status(400).send({ msg: "Not found!!!" })
        }
        res.status(200).send({ data: courses })
